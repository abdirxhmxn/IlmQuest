...
function findLegacyUniqueIndexes(indexes) {
  const canonicalNames = new Set([
    ...
    "platform_superadmin_username_unique",
    "platform_superadmin_email_unique"
  ]);
...
async function createPlatformIndexWithFallback(indexSpec, name, fieldName) {
  const withNe = {
    name,
    unique: true,
    partialFilterExpression: {
  ...
  try {
    await User.collection.createIndex(indexSpec, withNe);
    console.log(`Ensured platform index: ${name} (using $ne).`);
    return;
  } catch (err) {
    const msg = String(err?.message || "");
    const unsupportedPartial = msg.includes("Expression not supported in partial index");
    ...
    await User.collection.createIndex(indexSpec, withGt);
    console.log(`Ensured platform index: ${name} (fallback using $gt for server compatibility).`);
  }
}
...
async function createScopedIndexes() {
  ...
  await createPlatformIndexWithFallback(
