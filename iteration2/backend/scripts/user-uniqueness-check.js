...
function hasPlatformCanonicalUnique(indexes, name, keyPattern, field) {
  return indexes.some((idx) => {
    if (!idx.unique) return false;
    if (idx.name !== name) return false;
    if (!keyEq(idx.key, keyPattern)) return false;
    const p = idx.partialFilterExpression || {};
    if (p.deletedAt !== null) return false;
    if (p.role !== "superAdmin") return false;
    const fieldFilter = p[field] || {};
    const hasTypeString = fieldFilter.$type === "string";
    const hasNonEmptyConstraint = fieldFilter.$ne === "" || fieldFilter.$gt === "";
    return hasTypeString && hasNonEmptyConstraint;
...
async function main() {
  ...
  assert(
    hasPlatformCanonicalUnique(indexes, "platform_superadmin_username_unique", { role: 1, userNameNormalized: 1 }, "userNameNormalized"),
    "Platform superadmin username unique index is missing."
  );
  assert(
    hasPlatformCanonicalUnique(indexes, "platform_superadmin_email_unique", { role: 1, emailNormalized: 1 }, "emailNormalized"),
    "Platform superadmin email unique index is missing."
  );
