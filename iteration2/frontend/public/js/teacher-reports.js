(function initTeacherReportsPage() {
  ...
  async function refreshStats() {
    const response = await fetch('/teacher/reports/stats', {
      method: 'GET',
      ...
      if (recent.length === 0) {
        ...
        recent.slice(0, 8).forEach((entry) => {
          ...
          if (entry.generatedBy) {
            left.appendChild(document.createTextNode(` — by ${entry.generatedBy}`));
          }
    ...
    try {
      ...
      if (!response.ok || !contentType.includes('application/pdf')) {
        ...
        try {
          ...
          if (response.status === 403) message = 'You do not have access to generate a report for this target.';
          else if (response.status === 503) message = 'LaTeX compiler missing on server. Install pdflatex (TeX Live).';
        }
